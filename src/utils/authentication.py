...
class Authentication:
    ...
    def first_login(self, username: str, password: str) -> None:
        ...
        input_password = maskpass.askpass(
            prompt = Config.default_password_prompt, 
            mask="*"
        )
        if input_password != password:
            print(Config.wrong_password_prompt)
            self.invalid_login()
        else:
    ...
    def login(self) -> None:
        ...
        while True:
            ...
            if Authentication.max_login_attempts == 0:
                ...
                if not user_data:
                    print(Config.details_does_not_exist_prompt.format(username))
                    self.invalid_login()
                else:
